...
import { waitForNone } from '@console/internal-integration-tests';
...
export const isLoaded = () => browser.wait(until.and(waitForNone($$('.co-m-loader')), until.visibilityOf(saveButton))).then(() => browser.sleep(1000));

const getValue = () => (window as any).monaco.editor.getModels()[0].getValue();
export const getEditorContent = async(): Promise<string> => await browser.executeScript<string>(getValue);
